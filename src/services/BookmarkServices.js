...
exports.getBookmarkByUserId = async (body) => {
    return await Bookmarks.find({ bookmarkUserId: stringToObjectId(body.user._id), questionId: stringToObjectId(body.questionId) })
        .then((response) => {
            return response;
        })
        .catch((error) => {
...
exports.addNewBookmark = async (body) => {
    return await Bookmarks.findOneAndUpdate({bookmarkUserId: stringToObjectId(body.user._id)}, { $push: { questionId: stringToObjectId(body.questionId) } }, { new: true, upsert: true })
        .then((response) => {
            return [{
                code: 200,
                status: "OK",
...
exports.removeExistingBookmark = async (body) => {
    return await Bookmarks.findOneAndUpdate({bookmarkUserId: stringToObjectId(body.user._id)}, { $pull: { questionId: stringToObjectId(body.questionId) } }, { new: true })
        .then((response) => {
            return [{
                code: 200,
                status: "OK",
