...
exports.login = async (request, body) => {
  if (body.hasOwnProperty('loginDate') && Validation.isEmpty(body.loginDate.toString().trim())) {
    ...
    return await Users.find(userData)
      .then(async (response) => {
        if (response.length > 0) {
          if (response[0].active) {
            userData = {
              lastLogin: body.loginDate,
              ipAddress: Validation.isEmpty(body.ipAddress)
                ? body.ipAddress
              ...
              return [
                {
                  ...
                  status: 'OK',
                  message:
                    'Email verified. Looks like, We are facing some issue in mail sending. Please try again after an hour.',
                },
              ];
            }
          })
...
exports.resetPassword = async ({ email, newPassword }) => {
  return await Users.findOneAndUpdate({ email }, { password: newPassword }, { new: true })
    .then(async (response) => {
      if (response._id) {
...
exports.changePassword = async ({ email, oldPassword, newPassword }) => {
  return await this.confirmOldPassword(email, oldPassword)
    .then(async (response) => {
