  ...
  return await new Users(newUser)
    ...
    .then(async (resp) => {
      ...
      if (resp._id) {
        ...
        return await mail
          .sendMail(resp.email, subject, body)
          .then((sentMail) => {
      ...
      let res = [
        {
          code: 100,
          status: 'ERROR',
          message: error.message.includes('email_1 dup key')
            ...
            return [
              {
                ...
                status: 'OK',
                message:
                  'User updated successfully. Looks like, We are not able to get the updated data due to internal network issue.',
                data: [response],
              },
            ];
          });
